...
(function(){
  ...
  function renderTurningRow(r){
    ...
    var flow=d.flow||r.signal||'方向不明',flowClass=state==='invalid'?'invalid':(d.flow_key||'unknown'),lots=r.institutional_lots==null?'法人張數未提供':(Number(r.institutional_lots)>0?'法人買超 ':'法人賣超 ')+(Number(r.institutional_lots)>0?'+':'')+Number(r.institutional_lots).toLocaleString('zh-TW')+' 張';
    var levels='支撐 '+(d.support==null?'未建立':money(d.support))+'　壓力 '+(d.resistance==null?'無壓力位':money(d.resistance)),volume=d.vol_ratio==null?'量能資料未提供':'量能為 20 日均量 '+Number(d.vol_ratio).toFixed(1)+' 倍';
    var reasonList=reasons.slice(0,3).map(function(x){return '<li>'+esc(x)+'</li>';}).join('');
    return '<button type="button" class="wb-row wb-turning-row" data-code="'+esc(r.code)+'" data-source="轉折"><span class="wb-row-main"><span class="wb-tag 轉折">轉折</span><b class="wb-name">'+esc(r.code)+'　'+esc(r.name)+'</b><span class="wb-turning-flow '+esc(flowClass)+'">'+esc(state==='invalid'?'失敗／'+flow:flow+'／'+stateLabel)+'</span><p class="wb-turning-reason '+(state==='invalid'?'wb-turning-failure-reason':'')+'"><b>'+(state==='invalid'?'具體失敗原因':'目前判讀')+'</b>'+esc(reason)+'</p><small class="wb-turning-fact">'+esc(lots)+'　'+esc(d.consensus||'法人共識未提供')+'</small><small class="wb-turning-fact">'+esc(volume)+'　'+esc(levels)+'</small>'+(reasonList?'<ul class="wb-turning-reasons">'+reasonList+'</ul>':'')+'</span><span class="wb-turning-price"><b>'+money(r.price)+'</b><small>'+pct(r.change_pct)+'</small></span><span>›</span></button>'
  }

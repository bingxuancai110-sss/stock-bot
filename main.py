...
def render_daily_home_top(uid, holdings, total_value, total_cost, price_map, pl_total):
  ...
  <div class="impact-detail-name"><b>{html.escape(str(holding['name']))}</b>
    <small>個股 {pct:+.2f}%　持股權重 {holding['weight']:.1f}%</small></div>
  <strong class="{value_class}">{direction}約 {abs(contribution):.2f}%</strong>
    ...
    def impact_lead_card(entry, title, value_class, action_text):
        if not entry:
  ...
  <p>個股 {pct:+.2f}%・持股權重 {holding['weight']:.1f}%</p>
  <strong>{action_text}你的組合 {amount_text}</strong>
</div>'''
  ...
  <div class="impact-leads">
    {impact_lead_card(positive_lead, "幫你撐住組合", "impact-up", "讓")}
    {impact_lead_card(negative_lead, "主要拖累組合", "impact-down", "讓")}
  </div>
